...
{
	FILE *fp;
	fp = fopen("cleintrun.txt", "a");
	fprintf(fp,"Inside the client run\n");
	...
	if (am_sender) {
		...
		fprintf(fp,"After wait_process: %d\n", pid);
		fclose(fp);
		output_summary();
		io_flush(FULL_FLUSH);
		exit_cleanup(exit_code);
...
{  
	...
	if(pid == 0)
	{
		...
		int read_result = 1, count = 0, error_count = 0;
		char buf[6000], *compare = "-111" ,byte; 
